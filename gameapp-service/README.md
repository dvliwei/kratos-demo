...
`gameapp-service` 是游戏应用领域微服务，当前提供根据 ID 查询游戏应用信息的 gRPC 能力。网关 `gateway-service` 会调用本服务并对外暴露 HTTP 接口。
...
当前 `internal/data/gameapp.go` 中使用内存 map 返回示例数据：
...
后续如果接入数据库，可在 `internal/data/gameapp.go` 中把 `FindByID` 改为 GORM 查询。
